document.addEventListener('DOMContentLoaded', (event) => {
  ...
  window.onload = async function() {
    data = await d3.csv("boxplot/data.csv");
  ...
  data2 = await d3.csv("boxplot/data2.csv");
...
function updateLineChartRange() {
  ...
  function addLineChartRangeToDisplay(range) {
    ...
    deleteButton.addEventListener('click', () => {
      ...
      const index = selectedLineChartRanges.indexOf(range);
      if (index > -1) {
        selectedLineChartRanges.splice(index, 1);
      }
